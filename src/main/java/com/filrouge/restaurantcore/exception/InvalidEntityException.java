package com.filrouge.restaurantcore.exception;

import java.util.List;

import lombok.Getter;

/**
 * Exception when an entity is invalid.
 * @author Hermann
 *
 */
public class InvalidEntityException extends FunctionnalException {

	private static final long serialVersionUID = -2549085097027620141L;

	@Getter
	private ErrorCodes errorCode;

	@Getter
	private List<String> errors;

	public InvalidEntityException(String message) {
		super(message);
	}

	public InvalidEntityException(String message, Throwable cause) {
		super(message, cause);
	}

	public InvalidEntityException(String message, Throwable cause, ErrorCodes errorCode) {
		super(message, cause);
		this.errorCode = errorCode;
	}

	public InvalidEntityException(String message, ErrorCodes errorCode) {
		super(message);
		this.errorCode = errorCode;
	}

	public InvalidEntityException(String message, ErrorCodes errorCode, List<String> errors) {
		super(message);
		this.errorCode = errorCode;
		this.errors = errors;
	}

}
